...
const UserStats = () => {
  ...
  const normalizeData = (arrayOfObjects) => {
    ...
    const categories = [...new Map(arrayOfObjects.map(item => [item[key], {category: item.category}])).values()]
  
    const submissions = categories.map((x) => {
      return {...x, submissions: arrayOfObjects.filter(y => y.category === x.category)}
    }  )
